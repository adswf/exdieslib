...
// pre-computed
// Existen 20 keys (0-19) + -1 (no encryption)
...
// key_index = segundo argumento
// pointer de struct check
// offset = 0

void camellia_decrypt128(unsigned long key_index,
                         unsigned char* buff,
                         unsigned long offset)

{
    // array de unsigned long de tamaño 4 cuyos elementos son 0
    unsigned long  dst_block[4] = { 0 };
    // pointer unsigned lon
    unsigned long* src_block = (unsigned long*)buff;

    // key = pointer a dirección de key[19] (con los 54)
...
// Constante estática = 16
static const unsigned long DECRYPT_BLOCK_LENGTH = 16;


// toma una variable fd, key_index, puntero, length y offset (0)
// fd = resultado de open_or_die
// key_index = segundo argumento de 0 a 19
// buff = puntero de struct check
// len = size of check
...
// Extract con fd, key_index, prefix, offset, lenght y flatten
// read_decrypt
// 
            ...
            bool          flatten)
{
    ...
    if (memcmp(hdr.signature, "LIB", 3) == 0) {
        ...
        if (is_mgf) {
            filename = as::get_file_prefix(prefix, is_mgf) + ".png";
            memcpy(buff, "\x89PNG\x0D\x0kA\x1A\x0A", 8);
        }
...
int main(int argc, char** argv) {
    ...
    // Caso insuficientes argumentos (ERROR)
    if (argc < 3) {
        fprintf(stderr, "exdieslib v1.81 by asmodean\n\n");
        ...
        fprintf(stderr, "\tkey_index = 18 -> Electro Arms -Realize Digital Dimension- (v2)\n");
        fprintf(stderr, "\tkey_index = 19 -> 相州戦神館學園 八命陣 (v2)\n");
    ...
    // Variable string in_filename = primer argumento
    string        in_filename = argv[1];
    // Variable unsigned long key_index = entero del segundo argumento
    // sin el espacio de adelante
    unsigned long key_index = atol(argv[2]);
    // Variable booleana flatten = cuarto argumento si ! tercer argumento
    // es igual a "-flatten"
    bool          flatten = argc > 3 && !strcmp(argv[3], "-flatten");



    // aplicación de la función open_or_die sobre filename (archivo .dat)
    int fd = as::open_or_die(in_filename, O_RDONLY | O_BINARY);

    // Crear una structura check de tipo LIBPHDR
    LIBPHDR check;
    // Aplicar la función read_decript a las variables fd, key_index, &check,
    // size of check y 0
    read_decrypt(fd, key_index, &check, sizeof(check), 0);


    // si ! comparación de los 4 primeros bytes de check.signature son
    // iguales a "LIBP"
